...
class TRPO(multiprocessing.Process):
    ...
    def makeModel(self):
        ...
        with tf.variable_scope("policy"):
            h1 = fully_connected(self.obs, self.observation_size, self.hidden_size_1, weight_init, bias_init, "policy_h1")
            h1 = tf.nn.relu(h1)
    ...
    def loadModel(self,step):
        if self.saver is not None:
            self.saver.restore(self.session,"models/model-"+str(step))
            print "Model loaded successfully"
    def saveModel(self,step):
        if self.saver is not None:
            self.saver.save(self.session,"models/model",global_step=step)
    def act(self,obs):
